...
All mods go in one `mods` attrset keyed by [Modrinth](https://modrinth.com) slug. Empty `{}` includes the jar. Attrsets with fields configure the mod (mod modules read these and generate config files). The mod catalog (`slug -> { url, hash }`) is set automatically by the image base and version overlays from JSON files generated by `python3 tools/update-mods.py`.
