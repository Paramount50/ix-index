...
//!
//! One thin async entry point, [`search`], which marshals Python arguments into
//! a [`semantic_search_core::Query`], runs the index-then-search pipeline, and
//! returns each hit as a plain Python dict. All indexing, dedup, and search
//! logic lives in the core crate; this module only converts at the boundary.
//!
...
use semantic_search_core::{
    Config, DEFAULT_STORE, DisplayHit, MixedbreadStore, Query, SearchOptions,
};
...
/// Index the checkout at `path` (unless `no_sync`) and search it.
///
...
fn search(
    ...
    // lifetimes in `index_and_search`'s generic bounds.
...
async fn run_search(args: SearchArgs) -> semantic_search_core::Result<Vec<DisplayHit>> {
    ...
    semantic_search_core::index_and_search(&store, &query, &Config::default(), |_, _| {}, |_| {})
        .await
}
