...
let
  checkedDistribution = errors.assertEnum {
    name = "ix.languages.java.distribution";
    value = distribution;
    valid = validDistributions;
  };

  jdkTable = errors.requireAttr {
    context = "ix.languages.java: distribution table";
    attrset = jdksFor pkgs;
    key = checkedDistribution;
  };
in
errors.requireAttr {
  context = "ix.languages.java: unknown version for distribution '${checkedDistribution}'";
  attrset = jdkTable;
  key = version;
}
